...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">Lista de Prisioneros</h1>
    <a href="{{ route('prisioneros.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
        Nuevo Prisionero
    </a>
</div>
...
<form method="GET" class="bg-white p-4 rounded-lg shadow mb-6">
    <div class="flex gap-4 items-end">
        <div class="flex-1">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Nombre, apellido, identificación o celda..."
        ...
        <a href="{{ route('prisioneros.index') }}" class="bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500">
            Limpiar
        </a>
    </div>
</form>
...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <table class="w-full">
        ...
        <tbody>
            @forelse($prisioneros as $prisionero)
            <tr class="border-b hover:bg-gray-50">
                ...
                <td class="py-3 px-4">
                    <a href="{{ route('prisioneros.show', $prisionero) }}" class="text-blue-600 hover:underline mr-2">Ver</a>
                    <a href="{{ route('prisioneros.edit', $prisionero) }}" class="text-green-600 hover:underline mr-2">Editar</a>
                    <form action="{{ route('prisioneros.destroy', $prisionero) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-red-600 hover:underline" onclick="return confirm('¿Está seguro?')">
                            Eliminar
                        </button>
                    </form>
                </td>
            ...
            @empty
            <tr>
                <td colspan="6" class="py-8 text-center text-gray-500">No hay prisioneros registrados</td>
            </tr>
            @endforelse
        </tbody>
    </table>
